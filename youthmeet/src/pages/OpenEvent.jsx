export default function OpenEvent() {
    ...
    const interested = 49;
    const price = 0;
    const tags = ["Board Games", "Friends", "Dice"];
  
    const getDisplayPrice = () => (price > 0 ? `${price}kr` : "FREE");
  
    return (
      <div className="z-40 flex flex-col md:flex-row items-center md:items-start justify-center overflow-hidden shadow-xl flex-wrap p-4">
        ...
        <div className="flex flex-col md:items-start md:mr-8">
          ...
          <h1 className="font-oranienbaum text-3xl md:text-4xl py-4 text-center md:text-left">DnD Meeting</h1>
        </div>
        ...
        <div className="flex flex-col p-3">
          ...
          <div className="flex items-center p-2 font-lato">
            ...
            <p className="pl-2 text-lg">{tags.join(" | ")}</p>
          </div>
        </div>

        <button className="p-2 mt-4 md:mt-0 md:self-start bg-primary rounded-xl w-full md:w-auto text-2xl font-lato">
          Join Event | {getDisplayPrice()}
        </button>
      </div>
    );
  }